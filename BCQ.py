...
class VAE(nn.Module):
    ...
    def forward(self, state, action):
        ...
        random = np.random.normal(0, 1, size=(std.size()))
        z = mean + std * torch.FloatTensor(random).to(device)
    ...
    def decode(self, state, z=None):
        ...
        if z is None:
            random = np.random.normal(
                0, 1, size=(state.size(0), self.latent_dim)
            )
            z = torch.FloatTensor(random).clamp(-0.5, 0.5).to(device)
...
class BCQ(object):
    ...
    def train(self, replay_buffer, iterations, batch_size=100, discount=0.99, tau=0.005):
        for it in range(iterations):
            ...
            with torch.no_grad():
                target_Q = (
                    reward + done * discount * self.value_target(next_state)
                )
            ...
            with torch.no_grad():
                ...
                target_V = self.critic(
                    state, self.actor(state, self.vae.decode(state))
                )
            ...
            for param, target_param in param_iter:
                target_param.data.copy_(
                    tau * param.data + (1 - tau) * target_param.data
                )

    def save(self, filename, directory):
        actor_path = '{}/{}_actor.pth'.format(directory, filename)
        critic_path = '{}/{}_critic.pth'.format(directory, filename)
        value_path = '{}/{}_value.pth'.format(directory, filename)
        vae_path = '{}/{}_vae.pth'.format(directory, filename)
